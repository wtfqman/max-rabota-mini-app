...
export function createAdsRouter(container: ApiContainer): Router {
  ...
  const service = new AdsService(repository, container.adService);
  ...
  router.post('/:adId/archive', requireAuth, validateRequest({ params: adIdParamSchema }), controller.hideMine);
